...
suite('GaiaRadio', function() {
  ...
  test('Checked state', function() {
    ...
    var firstShadowInput = inputs[0].shadowRoot.querySelector('input');
    var secondShadowInput = inputs[1].shadowRoot.querySelector('input');
    assert.equal(firstShadowInput.checked, false);
    assert.equal(secondShadowInput.checked, true);

    simulateClick(inputs[0]);
    assert.equal(inputs[0].checked, true);
    assert.equal(inputs[1].checked, false);
    assert.equal(firstShadowInput.checked, true);
    assert.equal(secondShadowInput.checked, false);
  });
