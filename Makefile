...
$(NODE_MODULES_CACHEDIR)/$(NODE_MODULES_REV): gaia_node_modules.revision
	...
	-cd "$(NODE_MODULES_CACHEDIR)" && $(DOWNLOAD_CMD) https://github.com/mikehenrty/gaia-node-modules/tarball/$(NODE_MODULES_REV)
...
ifeq "$(NODE_MODULES_SRC)" "modules.tar"
	$(TAR_WILDCARDS) --strip-components 1 -x -m -f $(NODE_MODULES_SRC) "mikehenrty-gaia-node-modules-*/node_modules"
else
	rm -fr node_modules
